...
function normalizeVAR(vars?: number[]): number[] {
...
function parseVars(varsStr: string): number[] {
...
async function callRenderer(
  ...
  width: number,
  height: number
): Promise<{ pngBytes: Buffer; runtimeHash: string }> {
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          if (contentType.includes('image/png')) {
            const runtimeHash = res.headers['x-runtime-hash'] as string || sha256(SDK_VERSION);
          ...
          try {
            const json: RenderResponse = JSON.parse(responseBuffer.toString('utf-8'));
            if (json.error) {
...
async function runCommand(file: string, options: {
  out: string;
  seed: number;
  vars: string;
  width: number;
...
async function replayCommand(snapshotFile: string, options: {
  out: string;
  code?: string;
  renderer: 'local' | 'remote';
  endpoint: string;
  ...
  const snapshot: NexArtSnapshotV1 = JSON.parse(fs.readFileSync(snapshotFile, 'utf-8'));
...
async function verifyCommand(snapshotFile: string, options: {
  code?: string;
  renderer: 'local' | 'remote';
  endpoint: string;
}) {
...
yargs(hideBin(process.argv))
  .command(
    'run <file>',
    'Execute a Code Mode sketch and create snapshot',
    (yargs) => {
      return yargs
        .positional('file', {
    ...
    async (argv) => {
      await runCommand(argv.file as string, {
        out: argv.out,
        seed: argv.seed,
        vars: argv.vars,
        width: argv.width,
  ...
  .command(
    'replay <snapshot>',
    'Re-execute from a snapshot file',
    (yargs) => {
      return yargs
        .positional('snapshot', {
    ...
    async (argv) => {
      await replayCommand(argv.snapshot as string, {
        out: argv.out,
        code: argv.code,
        renderer: argv.renderer,
        endpoint: argv.endpoint,
  ...
  .command(
    'verify <snapshot>',
    'Verify a snapshot produces the expected output',
    (yargs) => {
      return yargs
        .positional('snapshot', {
          describe: 'Path to snapshot JSON file',
    ...
    async (argv) => {
      await verifyCommand(argv.snapshot as string, {
        code: argv.code,
        renderer: argv.renderer,
        endpoint: argv.endpoint,
      });
